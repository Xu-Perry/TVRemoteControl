...
final class AutoConnectViewModel {
    ...
    func restoreRememberedDevice(_ device: TVDevice?) {
        state.rememberedDevice = device
        if let device {
            state.selectedDevice = DiscoveredTVDevice(
    ...
    func select(_ device: DiscoveredTVDevice) {
        connectionTask?.cancel()
        ...
        connectionTask = Task { [weak self] in
            guard let self else { return }
            do {
                let transientDevice = device.tvDevice()
    ...
    func submitPIN() {
        ...
        connectionTask = Task { [weak self] in
            ...
            do {
                ...
                let credential = TVAuthCredential.cookie(authCookie)
                try await tvRemoteClient.testConnection(device: transientDevice, credential: credential)
                try Task.checkCancellation()
    ...
    private func upsert(_ device: DiscoveredTVDevice) {
