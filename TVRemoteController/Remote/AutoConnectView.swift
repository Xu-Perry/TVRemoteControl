...
struct AutoConnectView: View {
    ...
    private func canvasDeviceRow(_ device: DiscoveredTVDevice, y: CGFloat, height: CGFloat, status: String) -> some View {
        Button {
    ...
    private func responsiveDeviceRow(_ device: DiscoveredTVDevice) -> some View {
        Button {
            viewModel.select(device)
        } label: {
            HStack(spacing: 16) {
