# Research: TV Device Auto Discovery
...
## Decision: Model discovery as SSDP/UPnP-style local-network discovery with TV filtering

**Rationale**: TV TVs expose local-network control surfaces and the current architecture already names a discovery service and SSDP parsing as recommended boundaries. Discovery should collect candidate devices, fetch or parse enough metadata to identify TV TVs, deduplicate results, and expose app-friendly device identities.
...
## Decision: Keep protocol parsing and discovery service in `TVRemoteModules`
...
- Create a new package immediately: rejected because existing `TVRemoteCore` and `TVRemoteNetworking` already provide the right core/network split.
...
**Rationale**: Parser, deduplication, cancellation, retry, connection-state, and persistence behavior can be tested with fixtures and test doubles. Real discovery requires local-network conditions and a physical TV TV, so it must be documented as manual smoke instead of being hidden inside normal unit tests.
