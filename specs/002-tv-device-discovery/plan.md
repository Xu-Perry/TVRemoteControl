# Implementation Plan: TV Device Auto Discovery

**Branch**: `002-tv-device-discovery` | **Date**: 2026-05-01 | **Spec**: [spec.md](spec.md)
**Input**: Feature specification from `specs/002-tv-device-discovery/spec.md`

## Summary

Complete the automatic TV discovery chain shown in the Figma design: first launch, scan, device list, connect, connected-ready, no-devices recovery, and clear remembered connection. The implementation should keep discovery protocol parsing and connection-facing models testable in `Packages/TVRemoteModules`, while app target code owns SwiftUI state, view models, persistence coordination, and Figma-aligned screens.
...
**Testing**: Swift Testing via `import Testing`; package tests under `Packages/TVRemoteModules/Tests`; app tests under `TVRemoteControllerTests`  
...
**Constraints**: Discovery and connection are local-network behavior; automated tests must use test doubles and fixtures instead of requiring a real TV TV; real-device smoke is required before claiming end-to-end discovery works  
...
- Test-Driven Change: PASS. Protocol parsing, deduplication, cancellation-safe state transitions, remembered-device behavior, and no-devices recovery can be covered with automated tests. Real local-network discovery and a physical TV connection require documented manual smoke coverage.
...
**Structure Decision**: Reusable discovery models and parsing/network discovery boundaries belong in `Packages/TVRemoteModules` because they are stable, testable outside SwiftUI, and likely to remain useful beyond one screen. Figma-aligned screen state, navigation, retry/cancel behavior, and remembered-device coordination belong in the app target because they are product-flow specific.
...
- Test-Driven Change: PASS. The quickstart and contracts identify automated tests for parser/service/view-model behavior plus manual smoke validation for real TV and local-network behavior.
