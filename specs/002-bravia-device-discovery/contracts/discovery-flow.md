# Contract: BRAVIA Auto Discovery Flow
...
|-------|----------------------|-------------------|-----------------|
| First launch | Scan nearby devices or choose manual IP entry | Explains that BRAVIA TVs must be on the same network | Scan starts, or manual entry opens |
| Scanning | Cancel | Shows active scan progress | Devices found, no devices found, failure, or cancel |
| Devices found | Select a TV or rescan | Lists deduplicated BRAVIA TVs with names and readiness labels | Connection starts or scan restarts |
...
- Emit discovered BRAVIA device candidates during or at the end of the scan.
- Deduplicate candidates by stable identity or normalized network location.
- Complete with `devicesFound` when at least one BRAVIA TV is available.
- Complete with `noDevices` when the scan finishes without a BRAVIA TV.
...
- Device description parsing and BRAVIA filtering.
...
- Real BRAVIA discovery on the same Wi-Fi.
