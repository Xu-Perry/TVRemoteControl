# Data Model: BRAVIA Device Auto Discovery

## DiscoveredBRAVIADevice

Represents a BRAVIA TV found during local-network discovery.
...
- `port`: Control port, defaulting to the existing BRAVIA control port when discovery metadata does not provide one.
...
- `devices`: Current ordered list of discovered BRAVIA devices.
...
- `device`: Existing saved `SonyDevice` metadata.
