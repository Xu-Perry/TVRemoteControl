...
struct SSDPDeviceDescriptionParserTests {
    ...
    @Test func usesControlPortAndPreservesCustomFriendlyName() throws {
        let parser = SSDPDeviceDescriptionParser()
        let vendorSegment = TVProtocolEndpoint.vendorSegment
        let data = Data("""
        <root>
          <device>
            ...
            <serviceList>
              <service>
                <serviceType>\(TVProtocolEndpoint.scalarSearchTarget)</serviceType>
                <controlURL>http://192.168.0.196/\(vendorSegment)</controlURL>
              </service>
            </serviceList>
          </device>
          <av:X_ScalarWebAPI_DeviceInfo xmlns:av="urn:schemas-\(vendorSegment)-com:av"></av:X_ScalarWebAPI_DeviceInfo>
        </root>
        """.utf8)

        let device = try #require(try parser.parse(
            data: data,
            location: URL(string: "http://192.168.0.196:10165/\(TVProtocolEndpoint.deviceDescriptionPath)")!
        ))
