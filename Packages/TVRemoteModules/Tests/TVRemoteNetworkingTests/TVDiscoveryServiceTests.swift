...
struct TVDiscoveryServiceTests {
    @Test func filtersAndDeduplicatesCompatibleDevices() async throws {
        ...
        let service = TVDiscoveryService(ssdpClient: client) { url in
        ...
        var finishedDevices: [DiscoveredTVDevice] = []
    ...
    @Test func emptySearchFinishesWithNoDevices() async throws {
        let client = MockSSDPClient(responses: [])
        let service = TVDiscoveryService(ssdpClient: client) { _ in Data() }
